...
public class FloatLoveEffect extends AbstractGameEffect {
    ...
    public FloatLoveEffect(float x, float y) {
        this.duration = MathUtils.random(0.5F, 1.0F);

        this.x = x + MathUtils.random(-50F * Settings.scale, 50F * Settings.scale);
        this.initialX = this.x;
        this.y = y;

        this.img = new TextureRegion(ImageHelper.heart.get((int) (Math.random() * 4)));

        this.color = Color.PINK.cpy();
        this.rotation = 0;
        this.scale = MathUtils.random(0.3F, 0.6F) * Settings.scale;
        // 初始化速度和随机偏移
    ...
    private void resetPositionAndSpeed() {
        // 垂直方向的速度 (上升速度)
        verticalSpeed = MathUtils.random(300, 400);

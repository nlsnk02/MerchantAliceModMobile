...
public class TreasureSmugglingUpgradePower extends AbstractPower {
    ...
    public TreasureSmugglingUpgradePower(AbstractCreature owner) {
        this.name = NAME;
        this.ID = POWER_ID;
...
//        this.region48 = new TextureAtlas.AtlasRegion(ImageMaster.loadImage(PATH48), 0, 0, 32, 32);
    ...
    public void atEndOfTurn(boolean isPlayer) {
        if (isPlayer) {
            addToBot(new GainGoldAction(10));
            this.addToBot(new LoseHPAction(this.owner, this.owner, 5, AbstractGameAction.AttackEffect.FIRE));
        }
    }

    public void updateDescription() {
        this.description = DESCRIPTIONS[0] + 10 + DESCRIPTIONS[1] + 5 + DESCRIPTIONS[2];
    }
}